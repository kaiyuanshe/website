...
import ContentLocaleFields from "@/components/ContentLocaleFields";
  ...
  return (
    <div className={styles.inputWithIcon}>
      ...
      <Input
        ...
        placeholder={
          eventMode === "线上活动"
            ? translateUiText("请输入会议链接")
            : translateUiText("请输入详细地址")
        }
...
const XInput = (props: InputProps) => {
  ...
  return (
    <div className={styles.inputWithIcon}>
      ...
      <Input
        {...props}
        placeholder={translateUiText("请输入推文链接")}
        className={styles.inputWithIconField}
      />
    </div>
  );
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      const updateEventRequest = {
        ...
        event_type: values.eventType as
          "meetup" | "ama" | "hackathon" | "workshop",
        ...
        translation_of: values.translationOf,
      };
  ...
  if (!loading && !event) {
    return (
      <div className={styles.error}>
        ...
        <Link
          href={
            router.query.event_type === "coscon" ? "/events/coscon" : "/events"
          }
          className={styles.backButton}
        >
          {router.query.event_type === "coscon"
            ? translateUiText("返回中国开源年会")
            : translateUiText("返回活动列表")}
        </Link>
      </div>
    );
  }
        ...
        <div className={styles.formGrid}>
          ...
          <div className={styles.leftColumn}>
            ...
            <Card className={styles.section}>
              ...
              <Form.Item
                label={translateUiText("活动标题")}
                name="title"
                rules={[{ required: true, message: "请输入活动标题" }]}
              >
                <Input
                  placeholder={translateUiText("请输入活动标题")}
                  className={styles.input}
                />
              </Form.Item>

              <Form.Item
                label={translateUiText("活动描述")}
                name="description"
            ...
            <Card className={styles.section}>
              <h2 className={styles.sectionTitle}>
                ...
                <LocalizedText>{"活动形式 & 类型"}</LocalizedText>
              </h2>

              <div style={{ display: "flex", gap: "1rem" }}>
                {/* 活动形式 Select */}
                <Form.Item
                  label={translateUiText("活动形式")}
                  name="eventMode"
                  rules={[{ required: true, message: "请选择活动形式" }]}
                  style={{ flex: 1 }}
                >
                  <Select
                    placeholder={translateUiText("请选择活动形式")}
                    options={[
                      { label: translateUiText("线上活动"), value: "线上活动" },
                      { label: translateUiText("线下活动"), value: "线下活动" },
                    ]}
                ...
                {/* 活动类型 Select */}
                <Form.Item
                  label={translateUiText("活动类型")}
                  name="eventType"
                  rules={[{ required: true, message: "请选择活动类型" }]}
                  style={{ flex: 1 }}
                >
                  <Select
                    placeholder={translateUiText("请选择活动类型")}
                    disabled={!!eventType}
                    options={[
                      {
                        label: translateUiText("社区活动"),
                        value: "community",
                      },
                      { label: translateUiText("开源年会"), value: "coscon" },
                    ]}
                  />
                </Form.Item>
                ...
                {eventType === "coscon" && (
                  <div className={styles.formRow}>
                    <Form.Item
                      label={translateUiText("活动配置")}
                      name="eventSetting"
                      rules={[{ required: true, message: "请选活动配置" }]}
                      className={styles.fixedWidthItem}
                    >
                      <Select
                        placeholder={translateUiText("请选择活动配置")}
                        value={eventSetting}
                        options={[
                          { label: translateUiText("自行配置"), value: 1 },
                          {
                            label: translateUiText("跳转到百格网站"),
                            value: 2,
                          },
                        ]}
                    ...
                    {eventSetting == 2 && (
                      ...
                      >
                        <Input
                          placeholder={translateUiText("请输入百格链接")}
                          className={styles.inputWithIconField}
                        />
                      </Form.Item>
                    )}
            ...
            <Card className={styles.section}>
              <h2 className={styles.sectionTitle}>
                ...
                <LocalizedText>{"时间和地点"}</LocalizedText>
              </h2>

              <div className={styles.formRow}>
                <Form.Item
                  label={translateUiText("开始日期")}
                  name="startDate"
              ...
              <div className={styles.formRow}>
                <Form.Item label={translateUiText("结束日期")} name="endDate">
                  <DatePicker className={styles.input} />
                </Form.Item>
                <Form.Item label={translateUiText("结束时间")} name="endTime">
                  <TimePicker className={styles.input} format="HH:mm" />
                </Form.Item>
              </div>
              ...
              <Form.Item
                label={
                  eventMode === "线上活动"
                    ? translateUiText("活动链接")
                    : translateUiText("活动地址")
                }
                name="location"
                rules={[
                  {
          ...
          <div className={styles.rightColumn}>
            ...
            <Card className={styles.section}>
              <h2 className={styles.sectionTitle}>
                ...
                <LocalizedText>{"活动封面"}</LocalizedText>
              </h2>
              <Form.Item
                name="cover"
            ...
            <Card className={styles.section}>
              <h2 className={styles.sectionTitle}>
                <Users className={styles.sectionIcon} />
                <LocalizedText>{"其他设置"}</LocalizedText>
              </h2>

              <Form.Item
                label={translateUiText("志愿者/讲师报名注册链接")}
                name="applyLink"
                rules={[
                  {
                    type: "url",
              ...
              >
                <Input
                  placeholder={translateUiText("请输入链接")}
                  className={styles.input}
                />
              </Form.Item>
              <Form.Item
                label={translateUiText("议题征集链接")}
                name="topicLink"
                rules={[
                  {
                    type: "url",
              ...
              <Form.Item
                label={translateUiText("议题课件提交链接")}
                name="coursewareLink"
                rules={[
                  {
                    type: "url",
            ...
            <Card className={styles.section}>
              <h2 className={styles.sectionTitle}>
                ...
                <LocalizedText>{"活动标签"}</LocalizedText>
              </h2>
