...
export default function VenuesPage() {
  ...
  const activeVenue = useMemo(() => {
    return (
      venues.find((v) => getUniqueId(v) === activeVenueId) ||
      (venues.length > 0 ? venues[0] : createEmptyVenue())
    );
  ...
  const updateVenue = (venueId: string, field: keyof Venue, value: any) => {
    setVenues(
      venues.map((v) =>
        getUniqueId(v) === venueId ? { ...v, [field]: value } : v,
      ),
    );
  };
              ...
              setVenues(
                venues.map((v) => {
                  if (getUniqueId(v) === venueId) {
                    return {
                      ...v,
                      agendas: v.agendas.filter(
                        (a) => getUniqueId(a) !== agendaId,
                      ),
                    };
                  }
                  return v;
      ...
      setVenues(
        venues.map((v) => {
          if (getUniqueId(v) === venueId) {
            return {
              ...v,
              agendas: v.agendas.filter((a) => getUniqueId(a) !== agendaId),
            };
          }
          return v;
        }),
        ...
        if (getUniqueId(v) === venueId) {
          return {
            ...
            agendas: v.agendas.map((a) =>
              getUniqueId(a) === agendaId ? { ...a, [field]: value } : a,
            ),
          };
        }
        return v;
                  ...
                  return {
                    ...
                    agendas: v.agendas.map((a) => {
                      if (getUniqueId(a) === agendaId) {
                        return {
                          ...a,
                          speakers: a.speakers.filter(
                            (s) => getUniqueId(s) !== speakerId,
                          ),
                        };
                      }
                      return a;
              ...
              if (getUniqueId(a) === agendaId) {
                return {
                  ...
                  speakers: a.speakers.map((s) =>
                    getUniqueId(s) === speakerId ? { ...s, [field]: value } : s,
                  ),
                };
              }
              return a;
  ...
  const handleSubmitAgendas = async () => {
    ...
    try {
      ...
      if (result.success) {
        message.success(
          `成功为会场 "${activeVenue.name}" 添加议程: ${newAgenda.topic}`,
        );
        ...
        if (latestResult.success && latestResult.data) {
          ...
          // 保持当前激活的会场
          const currentVenue = updatedVenues.find(
            (v) => v.ID === activeVenue.ID,
          );
          if (currentVenue) {
    ...
    return (
      <div className={`${styles.container} nav-t-top`}>
        <div className={styles.error}>
          <LocalizedText>{"未找到活动ID"}</LocalizedText>
        </div>
      </div>
    );
  }
    ...
    <div className={`${styles.container} nav-t-top`}>
      ...
      <div className={styles.contentWrapper}>
        <div className={styles.tabsBar}>
          ...
          >
            <LocalizedText>{"添加会场"}</LocalizedText>
          </Button>
        </div>
        ...
        <div className={styles.formCard}>
          <div className={styles.section}>
            <h3 className={styles.sectionTitle}>
              <LocalizedText>{"会场信息"}</LocalizedText>
            </h3>
            ...
            >
              <Form.Item
                name="name"
                label={translateUiText("会场名称")}
                rules={[{ required: true, message: "请输入会场名称" }]}
                className={styles.formGroup}
              >
                <Input placeholder={translateUiText("请输入会场名称")} />
              </Form.Item>

              <Form.Item
                name="address"
                label={translateUiText("地址")}
                rules={[{ required: true, message: "请输入会场地址" }]}
                className={styles.formGroup}
              >
                <Input placeholder={translateUiText("请输入会场地址")} />
              </Form.Item>

              <Form.Item
                name="description"
                label={translateUiText("描述")}
                className={styles.formGroup}
              >
                <TextArea
                  rows={4}
                  placeholder={translateUiText("请输入会场介绍")}
                />
              </Form.Item>

              <Form.Item
                name="producer"
                label={translateUiText("出品人")}
                rules={[{ required: true, message: "请输入出品人" }]}
                className={styles.formGroup}
              >
                <Input placeholder={translateUiText("请输入出品人")} />
              </Form.Item>

              <Form.Item
                name="volunteers"
                label={translateUiText("志愿者（用逗号分隔）")}
                className={styles.formGroup}
              >
                <Input placeholder={translateUiText("请输入志愿者")} />
              </Form.Item>
              ...
              <Form.Item className={styles.formGroupBtn}>
                <Button
                  ...
                  size="large"
                  icon={
                    activeVenue.ID === null ? <Sparkles size={16} /> : undefined
                  }
                  onClick={handleSaveVenue}
                >
                  {activeVenue.ID === null
                    ? translateUiText("生成会场")
                    : translateUiText("更新会场")}
                </Button>
              </Form.Item>
            </Form>
          </div>
        ...
        <div className={styles.formCard}>
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              >
                <LocalizedText>{"添加议程"}</LocalizedText>
              </Button>
            </div>
            ...
            {activeVenue.agendas.length === 0 ? (
              <div className={styles.emptyAgenda}>
                <p>
                  <LocalizedText>{"暂无议程，请添加第一个议程"}</LocalizedText>
                </p>
              </div>
            ) : (
              activeVenue.agendas.map((agenda, agendaIndex) => (
                <div key={getUniqueId(agenda)} className={styles.agendaCard}>
                  <div className={styles.agendaHeader}>
                    ...
                    >
                      <LocalizedText>{"删除"}</LocalizedText>
                    </Button>
                  </div>
                  ...
                  <div className={styles.timeRow}>
                    <div className={styles.timeGroup}>
                      ...
                      <DatePicker
                        ...
                        placeholder={translateUiText("选择开始时间")}
                        value={
                          agenda.start_time
                            ? dayjs(agenda.start_time, "YYYY-MM-DD HH:mm:ss")
                            : null
                        }
                        onChange={(time) =>
                          updateAgenda(
                    ...
                    <div className={styles.timeGroup}>
                      ...
                      <DatePicker
                        showTime={{ format: "HH:mm" }}
                        format="YYYY-MM-DD HH:mm:ss"
                        placeholder={translateUiText("选择结束时间")}
                        value={
                          agenda.end_time
                            ? dayjs(agenda.end_time, "YYYY-MM-DD HH:mm:ss")
                            : null
                        }
                        onChange={(time) =>
                          updateAgenda(
                  ...
                  <div className={styles.formGroup}>
                    <label className={styles.label}>
                      <span className={styles.required}>*</span>
                      <LocalizedText>{"主题"}</LocalizedText>
                    </label>
                    <Input
                      placeholder={translateUiText("请输入议程主题")}
                      value={agenda.topic || ""}
                      onChange={(e) =>
                        updateAgenda(
                          getUniqueId(activeVenue),
                  ...
                  <div className={styles.speakersSection}>
                    <div className={styles.speakersHeader}>
                      <label className={styles.label}>
                        <LocalizedText>{"嘉宾"}</LocalizedText>
                      </label>
                      <Button
                        type="dashed"
                        size="small"
                        ...
                        disabled={agenda.ID !== null}
                      >
                        <LocalizedText>{"添加嘉宾"}</LocalizedText>
                      </Button>
                    </div>
                    ...
                    {agenda.speakers.length === 0 ? (
                      ...
                      agenda.speakers.map((speaker) => (
                        ...
                        >
                          <div className={styles.speakerRow}>
                            <div className={styles.speakerField}>
                              <label className={styles.smallLabel}>
                                <span className={styles.required}>*</span>
                                <LocalizedText>{"姓名"}</LocalizedText>
                              </label>
                              <Input
                                placeholder={translateUiText("请输入姓名")}
                                value={speaker.name || ""}
                                onChange={(e) =>
                                  updateSpeaker(
                                    getUniqueId(activeVenue),
                            ...
                            <div className={styles.speakerField}>
                              <label className={styles.smallLabel}>
                                <span className={styles.required}>*</span>
                                <LocalizedText>{"职位"}</LocalizedText>
                              </label>
                              <Input
                                placeholder={translateUiText("请输入职位")}
                                value={speaker.title || ""}
                                onChange={(e) =>
                                  updateSpeaker(
                                    getUniqueId(activeVenue),
                          ...
                          <div className={styles.formGroup}>
                            <label className={styles.smallLabel}>
                              <LocalizedText>{"头像"}</LocalizedText>
                            </label>
                          ...
                          >
                            <LocalizedText>{"删除嘉宾"}</LocalizedText>
                          </Button>
                        </div>
                      ))
                    )}
          ...
          <div className={styles.submitSection}>
            ...
            >
              <LocalizedText>{"提交议程"}</LocalizedText>
            </Button>
            {!currentVenueGenerated && (
              <div className={styles.submitTip}>
                <LocalizedText>{"请先生成当前会场后再提交议程"}</LocalizedText>
              </div>
            )}
          </div>
        </div>
