...
export default function ExecutivePage() {
  ...
  const renderWorkingGroup = (group: WorkingGroup) => {
    ...
    const groupName = translateText(group.name);
    const roleTitle = (role: "组长" | "副组长" | "组员") =>
      locale === "en"
        ? `${groupName} ${translateText(role)}`
        : `${group.name}${role}`;

    allMembers.push(createPersonCard(group.leader, roleTitle("组长")));

    if (group.viceLeader) {
      group.viceLeader.forEach((viceLeader) => {
        allMembers.push(createPersonCard(viceLeader, roleTitle("副组长")));
      });
    }

    group.members.forEach((member) => {
      allMembers.push(createPersonCard(member, roleTitle("组员")));
    });
    ...
    return (
      <div
        key={group.name}
        id={generateId(group.name)}
        className={styles.workingGroup}
      >
        <h3 className={styles.groupTitle}>{groupName}</h3>
        <p className={styles.groupDescription}>
          <LocalizedText>{"介绍："}</LocalizedText>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.description}>
          ...
          <div className={styles.details}>
            <p>
              <LocalizedText>{"执行委员会可行使如下职权："}</LocalizedText>
            </p>
            <p>
              <LocalizedText>
        ...
        <div className={styles.sidebar}>
          <nav className={styles.navigation}>
            <h3 className={styles.navTitle}>
              <LocalizedText>{"工作组导航"}</LocalizedText>
            </h3>
