...
主分支的 CI 会运行 `npm run images:sync`，把 `public/` 下新增或内容发生变化的图片上传到 Cloudinary。图片在 Cloudinary 中使用稳定路径，文件内容没有变化时会自动跳过；从仓库删除图片不会自动删除 Cloudinary 中的副本。

正式成员头像建议放在 `public/img/members/`，并继续在 `members.ts` 中使用本地路径：

```typescript
{
  name: '新成员姓名',
  gender: '女',
  avatar: '/img/members/github-username.webp',
  details: []
}
```

推送到 `main` 后，CI 会先完成 Cloudinary 同步，再允许生产部署继续。生产环境启用 `CLOUDINARY_STATIC_ASSETS=true` 后，Next.js 会把这些本地图片路径重定向到 Cloudinary；本地开发仍直接读取 `public/`，因此不需要为了预览先上传。
...
可选配置 Actions Variable `CLOUDINARY_STATIC_FOLDER`，默认值为 `kyswebsite/static`。生产构建环境必须配置相同的 `CLOUDINARY_CLOUD_NAME`、`CLOUDINARY_STATIC_FOLDER` 和 `CLOUDINARY_STATIC_ASSETS=true`；当前 SSH 部署流程需要在服务器执行 `next build` 前导出这些变量。Cloudinary API Secret 只能使用不带 `NEXT_PUBLIC_` 前缀的服务端变量。
